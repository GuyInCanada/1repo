...
Commands:
  ...
  backup                       Backs up the revanced signature file
                               (revanced.keystore) to the internal storage
                               of your device.

  restore                      Restores the revanced signature file
                               (revanced.keystore) to the working directory
                               of ReVanced-builder.
...
backup_keystore() { #this is a specific use case, for people who are wanting to use their revanced.keystore file on, or move it to, another device.
  log "Copying revanced.keystore to internal Android storage. Do not lose this file."
  if [ -f "$RVB_DIR/revanced/revanced.keystore" ]; then
    cp $RVB_DIR/revanced/revanced.keystore /storage/emulated/0
  fi
}

restore_keystore() {
  log "Restoring revanced.keystore to the revanced working directory."
  if [ -f "/storage/emulated/0/revanced.keystore" ]; then
    cp /storage/emulated/0/revanced.keystore $RVB_DIR/revanced/revanced.keystore
  fi
}
